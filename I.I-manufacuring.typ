== Fabrication Description <fab-desciption>
As discussed earlier, our design makes extensive use of pre-fabricated components to simplify the manufacturing and assembly process. Below is a description of the steps taken to bring the design to completion.
...
#{
  ...
  [
    #figure(
      table(
        columns: 2,
        table.header([Component], [Manufacturing Process]),
        ..data.map(it => (
          [
            #it.at("Part") \
            #text(
              weight: "thin", 
              [#better-ref(
                label("drawing:" + it.Part.replace(" ", "-") + "-p1")
              )]
            )
          ], 
          it.at("Manufacturing Process")
        )).flatten()
      ),
      caption: [Fabrication Description Table]
    )
  ]
}
